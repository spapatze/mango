...
function watchEverything() {
    ...
    mangoInstance.SetDriverLog({}, { fromBlock: creationBlock, toBlock: 'latest' }).get(function (error, result) {
        if (!error) {
            console.log(result);
...
function setTemC() {
    ...
    var _temC = document.getElementById('temperature').value;
    console.log(_temC);
    mangoInstance.setTemC.sendTransaction(_temC, {from: web3.eth.accounts[1]});

}

function transferToLab() {

    var _lab = document.getElementById('lab').value;
    console.log(_lab);
    mangoInstance.transferToLab.sendTransaction(_lab , {from: web3.eth.accounts[1]});

}

function setDriverLog() {

    var _log = document.getElementById('log').value;
    console.log(_log);
    mangoInstance.setDriverLog.sendTransaction(_log , {from: web3.eth.accounts[1]});
